...
import { PlayabilityShield } from "../src/core/playability.js";
...
import type { AttackRequest, Vec3, WorkItem } from "../src/core/types.js";
...
test("playability shield never degrades protected gameplay classes", () => {
  const shield = new PlayabilityShield();
  assert.equal(shield.isProtected("COMBAT"), true);
  assert.equal(shield.shouldDegrade("COMBAT", true), false);
  assert.equal(shield.shouldDegrade("DECORATIVE", true), true);
  assert.equal(shield.priorityFor("BOSS"), "CRITICAL");
});

test("governor becomes critical under saturated pressure", () => {
  const governor = new AdaptivePerformanceGovernor();
  assert.equal(governor.evaluate({ queueRatio: 1, workRatio: 1, memoryRatio: 0, localGameplayActive: false }), "CRITICAL");
});
...
test("universal attack API uses one resolver path with cooldown, critical, damage and knockback", () => {
  ...
  const port: CombatExecutionPort = {
    validateTarget: (_request: AttackRequest) => { calls.push("validate"); return true; },
    applyDamage: (_request: AttackRequest, damage: number) => { calls.push(`damage:${damage}`); return true; },
    applyKnockback: (_request: AttackRequest, impulse: Vec3) => { calls.push(`knockback:${impulse.x}`); },
  };
  const result = api.execute(request, port);
  assert.equal(result.accepted, true);
  assert.equal(result.finalDamage, 15);
  assert.equal(result.critical, true);
  assert.deepEqual(calls, ["validate", "damage:15", "knockback:2"]);
